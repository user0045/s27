...
const Details: React.FC = () => {
  ...
  // Get content details based on type - moved outside useMemo to avoid hooks issue
  const getContentDetails = () => {
    ...
    if (content.content_type === 'Movie' && content.movie) {
      ...
      // For web series, fetch current season data from database dynamically
      if (content.web_series?.seasons && content.web_series.seasons.length > 0) {
        // Use the selected season data
        ...
        // Fetch season data dynamically using selected season
        const asyncFetchSeasonData = async () => {
          if (content.web_series?.season_id_list && content.web_series.season_id_list.length > 0) {
            const { supabase } = await import('@/integrations/supabase/client');
            const seasonId = content.web_series.season_id_list[selectedSeason - 1];
              ...
              try {
                ...
                if (!error && seasonData) {
                  return {
                    ...
                    release_year: seasonData.release_year || new Date().getFullYear(),
                    rating_type: seasonData.rating_type || 'Not Rated',
                    rating: seasonData.rating || 0,
                    duration: null,
                    directors: seasonData.director || [],
                    writers: seasonData.writer || [],
                    cast: seasonData.cast_members || [],
                    thumbnail_url: seasonData.thumbnail_url || '',
                    trailer_url: seasonData.trailer_url || '',
                    genres: content.genre || []
                  };
                }
              } catch (err) {
        ...
        // For now, fallback to web series level data
        return {
                ...
                {content.content_type === 'Movie' ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6 relative">
                      ...
                      {/* Season Dropdown for Web Series - Positioned below badges */}
                        ...
                        <div className="mt-20 mb-8">
                          <div className="relative z-50">
                            ...
                            {isSeasonDropdownOpen && (
                              <div className="fixed inset-0 z-50" onClick={() => setIsSeasonDropdownOpen(false)}>
                                <div 
                                  className="absolute bg-gradient-to-br from-black/95 via-[#0A7D4B]/25 to-black/95 backdrop-blur-sm border border-primary/30 rounded-lg mt-1 max-h-48 overflow-y-auto shadow-2xl min-w-[200px]"
                                  style={{
                                    top: '50%',
                                    left: '50%',
                                  ...
                                  {content.web_series.season_id_list.map((seasonId, index) => (
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        // Force re-render by resetting showAllDetails
                                        setShowAllDetails(false);
                                        // Force refresh of details and episodes
                      ...
                      {/* Episodes List - Positioned below season dropdown or badges */}
                      {currentSeasonEpisodes.length > 0 && (
                        <div className={`max-h-96 overflow-y-auto ${content.content_type === 'Show' ? 'mt-20' : 'mt-8'}`}>
